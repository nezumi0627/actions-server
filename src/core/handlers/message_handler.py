...
class MessageHandler:
    ...
    def __init__(self) -> None:
        ...
        self.command_map = {
            ...
            USER_INFO_COMMAND: self.command_handler.user_info,
    ...
    def handle_message(
        ...
        # elif type_value == NotificationType.NOTIFICATION_IMAGE:
        #     self._handle_image_message(works, payload, channel_no)
        # elif type_value == NotificationType.NOTIFICATION_VIDEO:
        #     return
        # elif type_value == NotificationType.NOTIFICATION_LOCATION:
    ...
    def _handle_text_message(
        ...
        # userinfo コマンドの処理を追加
        if text.startswith(f"{COMMAND_PREFIX}userinfo:"):
            self.command_handler.user_info(works, channel_no, text)
            return

        # search コマンドの処理を追加
        if text.startswith(f"{COMMAND_PREFIX}search:"):
            self.command_handler.search(works, channel_no, text)
            return
    ...
    def _handle_sticker_message(
        ...
        Args:
            ...
            channel_no: str
        """
        # works.send_text_message(channel_no, f"Sticker: {payload.sticker}")
    ...
    #         channel_no: str
    #     """
    #     works.send_image_message(channel_no, payload.image_url)
